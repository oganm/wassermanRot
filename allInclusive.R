...
plot.new()
venList=list(seed6=which(!is.na(finalFrame$seed6)),
             CAGED=which(finalFrame$isCaged))

venn.plot=venn.diagram(venList,filename=NULL,fill=c('orange','green'))
grid.draw(venn.plot)

##############################################################

#unique(finalFrame[,c('precName','isCaged')])
##phyper for seed6 group looking for isCaged enrichment
m=sum(unique(finalFrame[,c('precName','isCaged')])$isCaged)
a=dim(unique(finalFrame[,c('precName','isCaged')]))
n=a[1]

a=dim(unique(finalFrame[!is.na(finalFrame$seed6),c('precName','isCaged')]))
k=a[1]

a=dim(unique(finalFrame[!is.na(finalFrame$seed6)&(finalFrame$isCaged),c('precName','isCaged')]))
x=a[1]
...
a=dim(unique(finalFrame[!is.na(finalFrame$shortRNAClust),c('precName','isCaged')]))
k=a[1]

a=dim(unique(finalFrame[!is.na(finalFrame$shortRNAClust)&(finalFrame$isCaged),c('precName','isCaged')]))
x=a[1]
...
m=sum(!is.na(unique(finalFrame[,c('precName','isCaged','seed6')])$seed6))
a=dim(unique(finalFrame[,c('precName','isCaged')]))
n=a[1]

a=dim(unique(finalFrame[finalFrame$isCaged,c('precName','isCaged')]))
k=a[1]

a=dim(unique(finalFrame[finalFrame$isCaged&(!is.na(finalFrame$seed6)),c('precName','isCaged')]))
x=a[1]
